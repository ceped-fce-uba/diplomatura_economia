...
format:
  html:
    embed-resources: true  # Ensure resources are embedded
    code-fold: true
...
-   Aprender R no es una prueba de memoria. Siempre van a tener a mano scripts viejos o ajenos para guiarse, así como foros de ayuda y "chats" de IA.

-   No necesitan saber todo sobre R para usarlo de manera productiva. Buscar en Google no es hacer trampa, sino todo lo contrario: saber buscar y aprovechar los motores de búsqueda se convierte en una habilidad en si misma. Eso sí, hay que aprender a separar la paja del trigo, poder verificar si lo que encontramos hace lo que necesitamos.

-   Casi siempre hay muchas formas distintas de resolver un mismo problema

-   Podríamos poner un link a un google forms para que nos cuenten su experiencia con R u otro software estadístico, o alguna expectativa que tengan, o alguna cosa.
...
```         
-   ¿A qué famosa banda de rock argentina hace referencia el ejercicio anterior? (esto era para joderlos a ustedes, no se preocupen por favor...)

-   Pásen también este objeto a la función class() para ver qué clase de objeto es.
```

-   Intenten llamar, desde la consola (no en el script) la variable COSA (en máyusculas). ¿Qué ocurre?

### Métodos de acceso

-   Levantar alguna basesita se me ocurre, quizá la del SIPA ya que estamos.
-   Instalar, si no lo hicieron durante la clase, el paquete tidyverse, con la función `library()`
