...
-   Extensible a otros lenguajes y usos (como esta presentación, hecha con **Quarto**)
-   Más veloz
...
El caracter **unidireccional** de `<-` implica que es posible redefinir con claridad objetos ya creados en el ambiente de R, reemplazándolos por el contenido de otro. Por ejemplo:

```{r}
Y <- X
Y # En este caso Y fue redefinida con el contenido de X
```
También se utiliza, aunque en menor medida, el operador `->`, cuya única diferencia es que asigna en el sentido contrario. Esto es cómodo en algunas situaciones. Por ejemplo:

```{r}
X -> Y
Y # Aunque escritas al revés, de vuelta Y fue redefinida con el contenido de X
```
