...
Sobre el final de la última clase  final de la última clase se trabajo sobre la visualización del salario real. En dicho proceso, se buscó visualizar una serie de tiempo del ingreso real, incluyendo capas vinculadas a la información (títulos, fuente, etc) y estética o trama del gráfico (un `theme`).
...
De este modo, se puede avanzar en la complejización a través de incluir un tema especial. Para ello, se utilizará el paquete `ggthemes`, el cual incluye una gran variaedad de estéticas. 
...
En términos de la estética del fondo, R también permite agregar marcas contextuales. Por ejemplo, una línea discontinua horizontal y otra punteada, vertical. Esto puede lograrse mediante sobreescribir el gráfico pero agregando capaz geométricas que indiquen una línea, es decir, las funciones `geom_hline()` y `geom_vline()`.
...
ggplot(graf, aes(x = P47T, fill = MAS_500, w = PONDII)) +
...
ggplot(graf, aes(x= NIVEL_ED, y = P21, group = NIVEL_ED, fill = NIVEL_ED, w = PONDIIO )) +
  geom_boxplot()+
...
```{r}
ggplot(graf, aes(x = P47T, group = MAS_500, fill = MAS_500, w = PONDII)) +
  geom_density(alpha = 0.4) +
  scale_x_continuous(limits = c(0, perc_99),
                     breaks = seq(0, 2000000, by = 1000000)) +
  facet_wrap(~ sexo) +
  theme_minimal() +
  labs(title = "Ingreso Horario según tamaño de aglomerado y sexo",
       subtitle = `trimestre`,
       caption = "Fuente: Encuesta Permanente de Hogares",
       x = "Ingreso Total Individual",
       y = "Densidad",
...
Y DENSIDAD POR TIPO DE INGRESO LAB-NOLAB??

### Complejización de la distribución: graficos de violin vs. boxplot

```{r}
ggplot(graf, aes(y = P47T, x = sexo, fill = sexo, w = PONDII)) +
  geom_boxplot() +
  scale_y_continuous(limits = c(0, perc_99)) +
  theme_minimal() +
  labs(title = "Distribución del Ingreso Total Individual según sexo",
       subtitle = `trimestre`,
       y = "Ingreso Total Individual",
       x = "Sexo",
       caption = "Fuente: Encuesta Permanente de Hogares"
       ) 
```
...
```{r}
dia_no_simult_ocup <- enut %>% 
  filter(EDAD_SEL > 18) %>%
  filter(CONDICION_ACTIVIDAD== 1) %>% 
  mutate(SEXO_SEL = case_when(SEXO_SEL == 1 ~ "Mujeres",
                              SEXO_SEL == 2 ~ "Varones"),
         horas_totales = TSS_GRANGRUPO_PERSONALES + TSS_GRANGRUPO_TNR + TSS_GRANGRUPO_OCUPACIONYAUTOCONSUMO - TSS_ACT_DORMIR - TSS_ACT_COMERYBEBER
         ) %>% 
  mutate(dem_cuid = case_when(CANT_DEMANDANTES_TOTAL >0 ~ "Con demandantes de cuidado en el hogar",
                              CANT_DEMANDANTES_TOTAL == 0 ~ "Sin demandantes de cuidado en el hogar")) %>%
  group_by(SEXO_SEL,dem_cuid) %>% 
  summarise(act_pers = weighted.mean((TSS_GRANGRUPO_PERSONALES-TSS_ACT_DORMIR - TSS_ACT_COMERYBEBER),WPER),
            no_rem = weighted.mean(TSS_GRANGRUPO_TNR,WPER),
            rem = weighted.mean(TSS_GRANGRUPO_OCUPACIONYAUTOCONSUMO,WPER),
            total = weighted.mean(horas_totales,WPER)) %>% 
  #mutate(control = act_pers + no_rem + rem)
  mutate("Actividades personales" = act_pers/total,
         "Trabajo no remunerado" = no_rem/total,
         "Trabajo remunerado" = rem/total) %>% 
  select(-no_rem,-rem,-act_pers,-total) %>% 
  pivot_longer(!c("SEXO_SEL","dem_cuid"),names_to = "act", values_to = "valor") %>% 
  mutate(grupo = paste0(SEXO_SEL, " - ",dem_cuid))

dia_no_simult_ocup
```

```{r}
colores3 = c("#facf7d","#e5616e","#bda8c3" )

dia_no_simult_ocup %>% 
  mutate(dem_cuid = factor(dem_cuid),
         dem_cuid = fct_rev(dem_cuid)) %>% 
  ggplot(.,aes(x = SEXO_SEL, y = valor, fill = act))+
  geom_col(position = "stack")+
  coord_flip()+
  facet_wrap(~dem_cuid, nrow = 2,scales = "free")+
  theme_minimal()+
  labs(title = "¿Cómo se distribuye el día de las personas?",
       subtitle = "Personas ocupadas mayores de 18 años",
       y = "Porcentaje de las horas diarias",
       x = "",
       fill = "",
       caption = "Nota: Se excluye el tiempo dedicado a dormir, comer y beber")+
