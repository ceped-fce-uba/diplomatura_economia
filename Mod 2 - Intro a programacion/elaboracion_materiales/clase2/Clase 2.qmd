...
output-file: "Clase 2 - Manipulación de bases de datos I" 
format:
  html:
...
:::: parallax-container
::: {#parallax-image .parallax-image-container .parallax-scroll data-image="imgs/tidyverse.png"}
:::
::::
...
A partir de este momento, trabajaremos las funciones a estudiar volcadas sobre una base de datos creada a partir de [información](https://www.argentina.gob.ar/sites/default/files/trabajoregistrado_2411_estadisticas.xlsx) del Sistema Integrado Previsional Argentino ([SIPA](https://www.argentina.gob.ar/situacion-y-evolucion-del-trabajo-registrado)). En este caso tenemos una base preprocesada guardada en formato `.csv`, del inglés *comma-separated values*, lo cual se traduce a valores separados por coma. Es una forma muy eficiente (y común) de guardar información.
...
str(base_sipa) #str es una abreviatura de structure
```
```{r}
unique(base_sipa$Variable)
```
Función clave de dplyr (mejor meter un "De ahora en más full dplyr"):
```{r}
remuneracion_media <- filter(base_sipa,
                             Variable == "Remuneración promedio - con estacionalidad")
```


### Levantar archivos de excel

En la práctica, es posible que se encuentren más seguido en situación de levantar archivos de Excel. Esto tiene sus particularidades y todo un paquete dedicado a ello: `readxl`. Éste es parte de la familia del tidyverse pero por no ser del núcleo duro del ecosistema tienen que importarlo aparte. Verán que la sintaxis es similar y que su uso es intuitivo.

```{r}
library(readxl) #en general, pero nostros ya importamos el tidyverse

base_ipc <- read_excel("bases/ipc_ceped_data.xlsx", sheet = "Hoja1")
```

Podemos ver las primeras 5 filas de la tabla:
```{r}
head(base_ipc)
```
Una función clave del paquete `dplyr` del tidyverse es `select()`, cuya finalidad es seleccionar un subconjunto de columnas. En este caso vamos a seleccionar las únicas dos columnas que necesitamos de la base del Índice de Precios al Consumidor (IPC).
```{r}
ipc_mensual <- select(base_ipc, fecha, valor)
```

Otra función importante: `rename()`, con la cual renombramos columnas a partir de la sintaxis nombre_nuevo = nombre_viejo. Reescribamos la base del IPC mensual con este cambio:

```{r}
ipc_mensual <- rename(ipc_mensual, ipc = valor)
```

Ahora, estas dos acciones las podríamos haber hecho con una sola línea, lo cual es más elegante:
```{r}
ipc_mensual <- select(base_ipc, fecha, ipc = valor)
```


## Dataframes y Tibbles

Acá un repaso sobre qué son

## Pruebas SIPA

La opción automática para explorar un dataset es la función `summary()`, que arroja un resumen genérico.

```{r}
summary(SIPA)
```

Una alternativa, del paquete `dplyr` es `summarize()`, con la cual pueden calcular medidas de resumen de columnas específicas.

```{r}
SIPA %>%
  summarize(
    min_fecha = min(Período),
    max_fecha = max(Período),
    min_anio = min(year(Período)),
    max_anio = max(year(Período))
  )
```
...
::: {#ref-ceped .csl-entry}
```         
CEPED. (2022). Ceped.data. Portal de difusión de datos del Centro de Estudios sobre Población, Empleo y Desarrollo (CEPED-UBA), Universidad de Buenos Aires, Buenos Aires. Recuperado de <a href="https://www.ceped-data.shinyapps.io/">https://www.ceped-data.shinyapps.io/</a>
```
:::
:::::
